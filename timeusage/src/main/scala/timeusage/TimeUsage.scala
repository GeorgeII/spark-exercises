...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
      ...
      columnNames
        .filter( column =>
          List("t01", "t03", "t11", "t1801", "t1803")
            .exists(start => column.startsWith(start))
        )
    ...
    val leisures =
      columnNames
        .filterNot((primaryNeeds.toSet | workingActivities.toSet).contains)
        .filter( column =>
          List("t02", "t04", "t06", "t07", "t08", "t09", "t10",
               "t12", "t13", "t14", "t15", "t16", "t18")
            .exists(start => column.startsWith(start))
        )

    (primaryNeeds.map(column), workingActivities.map(column), leisures.map(column))
  }
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column =
      when(col("teage") > 55, "elder")
        .when(col("teage") > 23, "active")
        .otherwise("young")
        .as("age")
